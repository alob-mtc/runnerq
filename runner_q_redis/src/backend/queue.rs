...
use runner_q::storage::{
    ActivityEvent, ActivityEventType, ActivityResult, ActivitySnapshot, FailureKind,
    IdempotencyBehavior, QueuedActivity, ResultState, StorageError,
};
use runner_q::{ActivityPriority, ActivityStatus};
...
fn calculate_priority_score(
    ...
    let fifo_offset = (1_000_000.0 - timestamp_fraction) / 1_000.0;
...
async fn write_snapshot(
    ...
    fields.push(("status".to_string(), serde_json::to_string(&snapshot.status)?));
    fields.push(("priority".to_string(), serde_json::to_string(&snapshot.priority)?));
    fields.push(("activity_type".to_string(), snapshot.activity_type.clone()));
    fields.push(("payload".to_string(), serde_json::to_string(&snapshot.payload)?));
    fields.push(("created_at".to_string(), snapshot.created_at.to_rfc3339()));
    fields.push((
        "scheduled_at".to_string(),
        snapshot.scheduled_at.map(|dt| dt.to_rfc3339()).unwrap_or_default(),
    ));
    fields.push((
        "started_at".to_string(),
        snapshot.started_at.map(|dt| dt.to_rfc3339()).unwrap_or_default(),
    ));
    fields.push((
        "completed_at".to_string(),
        snapshot.completed_at.map(|dt| dt.to_rfc3339()).unwrap_or_default(),
    ));
    fields.push((
    ...
    fields.push(("timeout_seconds".to_string(), snapshot.timeout_seconds.to_string()));
    fields.push((
        "retry_delay_seconds".to_string(),
        snapshot.retry_delay_seconds.to_string(),
    ));
    fields.push(("metadata".to_string(), serde_json::to_string(&snapshot.metadata)?));
    fields.push((
    ...
    fields.push((
        ...
        snapshot.last_error_at.map(|dt| dt.to_rfc3339()).unwrap_or_default(),
    ));
    fields.push((
    ...
    fields.push(
        ("score".to_string(), snapshot.score.map(|s| s.to_string()).unwrap_or_default()),
    );
    fields.push((
        "lease_deadline_ms".to_string(),
        snapshot.lease_deadline_ms.map(|s| s.to_string()).unwrap_or_default(),
    ));
    fields.push((
    ...
    let _: () = conn.hset_multiple(&activity_key, &fields).await.map_err(redis_to_storage)?;
    let _: () = conn.expire(&activity_key, SNAPSHOT_TTL_SECONDS).await.map_err(redis_to_storage)?;
    Ok(())
}
...
pub(super) async fn load_snapshot(
    ...
    let snapshot_json: Option<String> = conn.hget(&activity_key, "snapshot").await.map_err(redis_to_storage)?;
    match snapshot_json {
        Some(json) => {
            let snapshot: InternalSnapshot = serde_json::from_str(&json)
                .map_err(|e| StorageError::Internal(format!("Failed to deserialize snapshot: {}", e)))?;
            Ok(Some(snapshot))
        }
        None => Ok(None),
    }
...
async fn record_event(
    ...
    let _: Option<String> = conn.xadd(&global_stream_key, "*", &[("event", payload.as_str())]).await.map_err(redis_to_storage)?;
    let _: i64 = redis::cmd("XTRIM")
        ...
        .await.map_err(redis_to_storage)?;
    let _: () = conn.expire(&global_stream_key, EVENT_TTL_SECONDS).await.map_err(redis_to_storage)?;
    let events_key = RedisBackend::activity_events_key(activity_id);
    let _: () = conn.rpush(&events_key, payload).await.map_err(redis_to_storage)?;
    let _: () = conn.ltrim(&events_key, -EVENT_MAX_LEN, -1).await.map_err(redis_to_storage)?;
    let _: () = conn.expire(&events_key, SNAPSHOT_TTL_SECONDS).await.map_err(redis_to_storage)?;
    Ok(())
}
...
async fn add_to_completed_set(
    ...
    let _: () = conn.zadd(&completed_key, &member, score).await.map_err(redis_to_storage)?;
    let ttl: i64 = conn.ttl(&completed_key).await.map_err(redis_to_storage)?;
    if ttl == -1 {
        let _: () = conn.expire(&completed_key, SNAPSHOT_TTL_SECONDS).await.map_err(redis_to_storage)?;
    }
    Ok(())
}
...
async fn ack_processing(
    ...
    let activity_key = RedisBackend::activity_key(activity_id);
    let member: Option<String> = conn.hget(&activity_key, "processing_member").await.map_err(redis_to_storage)?;
    if let Some(m) = member {
        let removed: i32 = conn.zrem(&processing_key, &m).await.map_err(redis_to_storage)?;
        if removed == 0 {
            warn!(activity_id = %activity_id, "ack_processing: member not found in processing ZSET");
        }
        let _: () = conn.hdel(&activity_key, &["processing_member", "lease_deadline_ms"]).await.map_err(redis_to_storage)?;
    }
    Ok(())
}
...
pub async fn enqueue(backend: &RedisBackend, activity: QueuedActivity) -> Result<(), StorageError> {
    ...
    if let Some(scheduled_at) = activity.scheduled_at {
        ...
        let _: () = conn.zadd(&scheduled_key, queue_entry, scheduled_at.timestamp()).await.map_err(redis_to_storage)?;
    ...
    let queue_entry = create_queue_entry(&activity);
    let score = calculate_priority_score(&activity.priority, activity.created_at, activity.retry_count);
    let _: () = conn.zadd(&queue_key, queue_entry, score).await.map_err(redis_to_storage)?;
    let mut snapshot = InternalSnapshot::from_queued(&activity);
    ...
    record_event(
        ...
        None,
        Some(json!({ "priority": activity.priority, "scheduled_at": activity.scheduled_at })),
    )
    .await?;
    info!(activity_id = %activity.id, activity_type = ?activity.activity_type, priority = ?activity.priority, score = %score, "Activity enqueued");
    Ok(())
}
    ...
    while start_time.elapsed() < timeout {
        ...
        if let Some(queue_entry) = claimed {
            ...
            let meta_fields = vec![
                ...
                ("lease_deadline_ms".to_string(), lease_deadline_ms.to_string()),
            ];
            let _: () = conn.hset_multiple(&activity_key, &meta_fields).await.map_err(redis_to_storage)?;
...
pub async fn ack_success(
    ...
    if let Some(data) = result {
        let activity_result = ActivityResult { data: Some(data), state: ResultState::Ok };
...
pub async fn ack_failure(
    ...
    if snapshot.max_retries == 0 || snapshot.retry_count + 1 < snapshot.max_retries {
        ...
        record_event(
            ...
            Some(worker_id),
            Some(json!({ "retry_count": snapshot.retry_count, "scheduled_at": scheduled_at, "error": error_message })),
        )
        .await?;
    ...
    let _: () = conn.rpush(&dead_letter_key, serde_json::to_string(&dead_letter_entry)?).await.map_err(redis_to_storage)?;
    record_event(
        &mut conn,
        backend,
        &activity_id,
...
pub async fn process_scheduled(backend: &RedisBackend) -> Result<u64, StorageError> {
        ...
        if redis.call('ZREM', KEYS[1], m) == 1 then
            ...
            if existing_score == false then
                ...
                if colon then
                    ...
                    if not score then score = now * 1000000
                    else score = tonumber(score); if not score then score = now * 1000000 end end
    ...
    for queue_entry in &moved_entries {
        if let Ok(activity) = parse_queue_entry(queue_entry) {
            let correct_score =
                calculate_priority_score(&activity.priority, activity.created_at, activity.retry_count);
            let current_score: Option<f64> = conn.zscore(&main_key, queue_entry).await.map_err(redis_to_storage)?;
            if current_score.map(|s| (s - correct_score).abs() > 0.001).unwrap_or(true) {
                let _: () = conn.zadd(&main_key, queue_entry, correct_score).await.map_err(redis_to_storage)?;
            }
            if let Ok(Some(mut snapshot)) = load_snapshot(&mut conn, &activity.id).await {
                ...
                let _ = record_event(
                    ...
                    ActivityEventType::Enqueued,
                    None,
                    Some(json!({ "priority": snapshot.priority, "scheduled_at": snapshot.scheduled_at, "from": "scheduled" })),
                )
                .await;
            }
        }
...
pub async fn requeue_expired(backend: &RedisBackend, batch_size: usize) -> Result<u64, StorageError> {
    let mut conn = get_conn(backend).await?;
    let processing_key = backend.processing_queue_key();
    let main_key = backend.main_queue_key();
    ...
    for _, m in ipairs(members) do
        if redis.call('ZREM', KEYS[1], m) == 1 then
            ...
            if existing_score == false then
                ...
                else redis.call('ZADD', KEYS[2], now, m) end
            end
        end
    end
    return moved_ids
...
pub async fn extend_lease(
    ...
    if current_deadline == false then return 0 end
    ...
    if new_deadline > 0 {
        if let Ok(Some(mut snapshot)) = load_snapshot(&mut conn, &activity_id).await {
            ...
            let _ = record_event(
                ...
                None,
                Some(json!({ "new_deadline_ms": new_deadline, "extend_by_ms": extend_ms })),
            )
            .await;
        }
    }
...
pub async fn store_result(
    ...
    let _: () = conn.set_ex(&result_key, result_json, RESULT_TTL_SECONDS).await.map_err(redis_to_storage)?;
    record_event(
        &mut conn,
        backend,
        &activity_id,
...
pub async fn get_result(
    ...
    let mut conn = get_conn(backend).await?;
    let result_key = RedisBackend::result_key(&activity_id);
    let result_json: Option<String> = conn.get(&result_key).await.map_err(redis_to_storage)?;
    match result_json {
        Some(json) => {
...
pub async fn check_idempotency(
    ...
    let existing_id_str: Option<String> = conn.get(&idempotency_key).await.map_err(redis_to_storage)?;
    let existing_id = existing_id_str
        .and_then(|s| Uuid::parse_str(&s).ok())
        .ok_or_else(|| StorageError::Internal("Idempotency key exists but activity_id not found".to_string()))?;
    let existing_snapshot = match load_snapshot(&mut conn, &existing_id).await? {
        Some(s) => s,
        None => {
            let _: () = conn
                .set_ex(&idempotency_key, activity.id.to_string(), IDEMPOTENCY_KEY_TTL_SECONDS)
                .await.map_err(redis_to_storage)?;
            return Ok(None);
        }
    };
