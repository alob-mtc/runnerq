...
use runner_q::storage::{
    ActivityEvent, ActivitySnapshot, DeadLetterRecord, PriorityBreakdown, QueuedActivity,
    QueueStats, StorageError,
};
...
pub async fn stats(backend: &RedisBackend) -> Result<QueueStats, StorageError> {
    ...
    let total_pending: u64 = conn.zcard(&queue_key).await.map_err(redis_to_storage)?;
    let processing_count: u64 = conn.zcard(&processing_key).await.map_err(redis_to_storage)?;
    let pending_critical: u64 = conn.zcount(&queue_key, 4_000_000.0, 4_999_999.0).await.map_err(redis_to_storage)?;
    let pending_high: u64 = conn.zcount(&queue_key, 3_000_000.0, 3_999_999.0).await.map_err(redis_to_storage)?;
    let pending_normal: u64 = conn.zcount(&queue_key, 2_000_000.0, 2_999_999.0).await.map_err(redis_to_storage)?;
    let pending_low: u64 = conn.zcount(&queue_key, 1_000_000.0, 1_999_999.0).await.map_err(redis_to_storage)?;
    let processing_critical: u64 = conn.zcount(&processing_key, 4_000_000.0, 4_999_999.0).await.map_err(redis_to_storage)?;
    let processing_high: u64 = conn.zcount(&processing_key, 3_000_000.0, 3_999_999.0).await.map_err(redis_to_storage)?;
    let processing_normal: u64 = conn.zcount(&processing_key, 2_000_000.0, 2_999_999.0).await.map_err(redis_to_storage)?;
    let processing_low: u64 = conn.zcount(&processing_key, 1_000_000.0, 1_999_999.0).await.map_err(redis_to_storage)?;
    let scheduled_count: u64 = conn.zcard(&scheduled_key).await.map_err(redis_to_storage)?;
    let dead_letter_count: u64 = conn.llen(&dead_letter_key).await.map_err(redis_to_storage)?;
    Ok(QueueStats {
...
pub async fn list_dead_letter(
    ...
    for entry in entries {
        match serde_json::from_str::<DeadLetterEnvelope>(&entry) {
            ...
            Err(e) => warn!(error = %e, "Failed to parse dead letter entry"),
        }
    }
    Ok(records)
}
...
pub async fn get_activity_events(
    ...
    for raw in raw_events {
        match serde_json::from_str::<ActivityEvent>(&raw) {
            ...
            Err(e) => warn!(error = %e, "Failed to parse activity event"),
        }
    }
    Ok(events)
}
    ...
    let stream = futures::stream::unfold(
        ...
        move |state| async move {
            ...
            let mut conn = match pool.get().await {
                Ok(conn) => conn,
                Err(e) => {
                    error!(queue_name = %qn, error = %e, "Failed to get Redis connection for event stream");
                    tokio::time::sleep(Duration::from_millis(1000)).await;
                    return Some((
                        Err(StorageError::Unavailable(format!("Redis connection error: {}", e))),
            ...
            let result: Result<redis::streams::StreamReadReply, redis::RedisError> = conn
                .xread_options(
                    &[key.as_str()],
                    &[last_id.as_str()],
                    &redis::streams::StreamReadOptions::default().count(50).block(1000),
                )
                .await;
            match result {
                Ok(reply) => {
                                ...
                                if field == "event" {
                                    if let redis::Value::Data(data) = value {
                                        if let Ok(json_str) = String::from_utf8(data.clone()) {
                                            if let Ok(event) = serde_json::from_str::<ActivityEvent>(&json_str) {
                                                return Some((
                                                    Ok(event),
                                                    (pool.clone(), key.clone(), qn.clone(), last_id),
                                                ));
                ...
                Err(e) => {
                    warn!(queue_name = %qn, error = %e, "Error reading from Redis stream");
                    tokio::time::sleep(Duration::from_millis(1000)).await;
                    Some((
                        Err(StorageError::Unavailable(format!("Redis stream error: {}", e))),
                        (pool.clone(), key.clone(), qn.clone(), last_id),
                    ))
                }
            }
